...
knitr::include_graphics(here("images","environmental-data-science-r4ds-general.png"))
```
...
First we need to load our data, which we created in the module on [joins and binds](08_joins.html), and then inspect what's there.
...
It generally appears that depth to groundwater is increasing over time (groundwater depletion), but a few clearly erroneous values > 250^[Info on why we know this is erroneous] feet are impairing the plot. Let's remove these values and re-plot.
...
Let's tie this back to our spatial data to answer the question, **"which areas have experienced the largest drop in groundwater levels over their historical period of record?"** 
Here are a few possible ways to constrain this analysis to make interpretation easier. 
...
library(glue) # package to help with pasting values/text together
...
<!-- Need to fix or add this dataset in order to view map below... -->

```{r preprocess nhd, eval = FALSE, echo=FALSE}
...
ggplot() +
  geom_sf(data = sac) +
  geom_sf(data = gwl_diff, aes(fill = diff), pch = 21, size=2.7, alpha=0.8) +
...
An EDA may generate tables, visualizations, text, and code that all encapsulate the greater meaning that you've derived from the data. An excellent, R-centric way to share the tables, visualizations, text, and code that result from your EDA is to use an RMarkdown (`.Rmd`) document, which is the topic of the [next module](11_intro_Rmarkdown.html).  

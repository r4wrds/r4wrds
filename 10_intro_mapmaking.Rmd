...
```{r plot-all-stations, eval=TRUE, echo=FALSE, out.height='110%'}

# plot, and make the points opaque and small
plot(all_gw_stations, max.plot = 1, pch = 16, cex = 0.1)
...
```{r mapviewOptions, include=FALSE}
library(mapview)
# may need to specify this option to get this to properly knit/print:
mapviewOptions(fgb = FALSE)
```

<!-- while I like the mapview maps, I feel like it may distract here...may be best to just use static maps and hide that code...focus just on the spatial join part, so show maps to demonstrate that it worked, but nothing beyond that. And I still think you need mapviewOptions(fgb=FALSE). I have the github most recent dev version and still the maps don't render in Chrome when expanding the knit lesson. They render just fine in the Viewer tab in Rstudio though...which is interesting. If we use fgb=FALSE, the files will be self contained and should host on the website more easily (though file sizes will be a bit bigger).
-->


```{r st-intersection2, out.width"100%"}

# filter to Sacramento county & pipe into mapview (will cover later)
stations %>% 
  filter(COUNTY_NAME == "Sacramento") %>% 
  mapview::mapview() # we will cover this later in the module
...
<!-- recommend adding notes on what planar units are and why its good practice, or at least a reference here -->

```{r st-intersection3, out.width"100%"}

# it's good practice to ensure your spatial data are in planar units
...
# perform the intersection and pipe into mapview 
st_intersection(all_gw_stations_3310, sac_3310) %>% 
  mapview::mapview()

```


There are many many more methods available beyond intersections, including area, distances, buffers, crops, voroni polygons, nearest neighbor calculations, convex hull calculations, centroid calculations, and much, much more. The list of operations within `{sf}` are shown below.  

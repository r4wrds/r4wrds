...
But what if we didn't have the county data already, or if we wanted to filter these data by a polygon that wasn't detailed in one of the columns of the dataframe? In this case, we use `st_intersection()` which is a **spatial join** that takes two arguments, an `sf` object to filter, and another `sf` object to filter by. If we pass it `all_gw_stations` and `sac` it will return all of the points in `all_gw_stations` that fall within `sac`.

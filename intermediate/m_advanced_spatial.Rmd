...
If we want to select more than a single point based on a threshold distance we can use a non-overlapping join and specify a distance. Here it's best to use a projected CRS.
...
# Plot temp
(gg_temp <- ggplot() + 
  geom_line(data = wTemp, aes(x = Date, y = Wtemp, col = site_no),
            size = .5) + 
  facet_wrap(~site_no) + 
  theme_minimal() + 
...
Next we can download a basemap layer to use in our map. For this we need to specify an area we can use. We can use the rivers layer we pulled previously using the `{dataRetrieval}` package. For this, we can use the `st_box()` to get an extent.
...
When plotting rasters, we can specify the resolution that gets plotted using `tmap_options(max.raster = c(plot=1e6, view=1e6))`. This helps speed up plotting when necessary.

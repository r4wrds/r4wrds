...
What makes a predictable development environment? What is a *development environment?*
...
After reviewing core introductory topics, we will discuss `.RProfile` and `.Renviron` files and when to use them. To review core best practices we recommend particular familiarity with the following concepts:
 ...
 - [Filenaming](https://r4wrds.com/intro/m_project_management.html#safe-file-naming)
 - [Relative filepaths and the **`{here}`** package](https://r4wrds.com/intro/m_project_management.html#file-paths)
 - [Best practices for project organization](https://r4wrds.com/intro/m_project_management.html#best-practices-organization-tips)
...
inspire_me <- function(animal){
  ...
  suppressMessages(
    ...
    repo <- "JakubPetriska/060958fd744ca34f099e947cd080b540"
    csv <- "raw/963b5a9355f04741239407320ac973a6096cd7b6/quotes.csv"
    quotes  <- readr::read_csv(glue::glue("https://gist.githubusercontent.com/{repo}/{csv}")  
  )
...
Although it may not appear powerful in this trivial example, when a project grows substantially large and complex, or when managing multiple sensitive passwords and access tokens, environmental variables are a standard approach that is widely used.

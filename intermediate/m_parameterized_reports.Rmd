...
In this module, we will focus on example use case 1, and build paramaterized reports for a set of geographic locations. Throughout this course, we've been working with groundwater elevation data across California counties. Let's imagine that we want to generate a report on groundwater level trends for a set of counties. Although the RStudio Interactive Development Environment (IDE) encourages knitting RMarkdown documents by clicking a button, we can also knit documents via: `knitr::knit()`. Iterating over `knit()` is the key to easily generating many paramaterized reports.


## `params`

A paramaterized .Rmd file takes in the YAML header a set of `params` which are bound into a named list called `params` and accessed with code from within the .Rmd file with `params$<paramater-name>`. For example, consider the example YAML:

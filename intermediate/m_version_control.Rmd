...
Then when we enter a `commit` message and click `Commit`, we'll end up with this, which tells us our commit worked...but it's still only *local*! Note we'll have a message saying our branch is "*`ahead of origin/jo_download_data by 1 commit.`*" That's ok, it just means we still need to **`push`** our changes up to the branch.
...
The final step is to **`push`** our changes up to the cloud. Click **`Push`** and hopefully we will get  message back like this. This means things worked...a final check would be go to Github and make sure the file is online!
...
The next step in this `version control` odyssey is to get <font color="#5f3dc4">**Jo's**</font> work into the **`main`** branch. Here we use a ***Pull Request***. This is a way to review the changes, check for any conflicts (if for instance, folks were working on the same file), and then **Merge** these changes into the **`main`** branch. Then we can delete the branch, and create another one to work on the next task, and so on.
...
```{r pr1, layout="l-body-outset", eval=TRUE, echo=FALSE}

include_graphics(here("images/git_pr1.png"))

```

Next we have an option to add some additional descriptions, comments, about what this PR (pull request) is doing, and why. We can tag a review, add labels, milestones, etc. These are all helpful for keeping track of what's done and what's not.

```{r pr2, layout="l-body-outset", eval=TRUE, echo=FALSE}
...
```{r pr3, layout="l-body-outset", eval=TRUE, echo=FALSE}
...
```{r pr4, layout="l-body-outset", eval=TRUE, echo=FALSE}
...
Now that <font color="#5f3dc4">**Jo**</font> has completed the first part of the team task, we want to move to the second task, which is cleaning the data. Thankfully <font color="#2b8a3e">**Mo**</font> is a great data/visualization analyst, and has a script that will clean up the code and visualize it for us. But first, <font color="#2b8a3e">**Mo**</font> needs to `pull` the changes that <font color="#5f3dc4">**Jo**</font> just merged into the `main` repository. There are two approaches to this. One is to use a `git fetch`, the other is a `git pull`. The main difference between the two: 
 ...
 - **`pull`**: This downloads the remote content and then immediately `merges` the content with your local state, but if you have pending work, this will create something called a **merge conflict**, but not to worry, these can be fixed!^[https://docs.github.com/en/github/collaborating-with-issues-and-pull-requests/resolving-a-merge-conflict-on-github]. 
 
So, <font color="#2b8a3e">**Mo**</font> needs to **`pull`** changes in to his branch from `main` and proceed with the task. Here we assume <font color="#5f3dc4">**Jo**</font> has already completed work on her branch to download data and clean and transform the data.
...
So we now have a `commit` history that might look something like this.
...
So, we've now walked through how to create branches, add or revise files, and then commit and push these changes to a shared repository for collaborative projects. Importantly, it's generally a good idea to always `pull`/`fetch` at the beginning of each work session so you have the most-up-to-date changes. And if something needs to be fixed, you can always pull a specific branch and make a change and pull request for that branch before pull requesting and merging back to the `main`. 
...
This is a simplified workflow, but it is very powerful. There are additional options within this workflow, like reverting to previous versions, pruning branches or git history, and more, but the core steps to successfully using version control really rely on the material covered above, with practice! 

## Additional Git Details

While there are many more details important to learning version control, many come with more practice and expertise. However, a few important tidbits:
...
<a href="index.html" class="btn btn-secondary" style="float: left">Previous module:<br> Project Management</a>
<a href="m_version_control.html" class="btn btn-secondary" style="float: right;">Next module:<br> Iteration</a>  

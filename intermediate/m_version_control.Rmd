...
So while we will cover version control with **`git`** and learn basic commands in this module it's important to remember:  
...
Developing the skills to organize and collaborate effectively (not work on the same thing at the same time to avoid conflicts), and integrate people and pieces together is really at the root of successful version control!

What we want you to come away with is a real understanding of not just the `git` commands (which are good to learn!), but how to implement version control so it's useful, not just another way to back things up.

(ref:xkcdGit) *Learn to leverage the power of git for things other than just a backup! (Comic by xkcd)*

```{r, fig.cap='(ref:xkcdGit)', out.width='50%', echo=FALSE}
...
There are a few main tasks that need to occur in the project, and some overlap in the code used to update and produce a status report.
...
**`git`** is a version control program that is installed on your computer with associated commands we use to interact with version controlled files. **Github** is a website that allows us to store/access/share our files as *repositories*. There is a very rich and complicated set of tools and functionality with git/Github, but we'll focus on the basics here.
...
Great! We should now have an RStudio project that is version controlled with `git`, and there is a local copy. Let's move to some basic commands, and then more complicated workflows.

### `add`, `commit`, `push`

There are only a few basic commands in `git`, which is good. The general process to follow is as follows:

 1. Do some work, add some files, etc. Next we need to stage or `add` that work so it can be *versioned* by `git`. We do this with `git add <my_file>` or check the **`Staged`** buttons in the RStudio `git` panel.
 ...
 3. `push` this up to the Github `remote` repository in the cloud! Now you have a point in time you can return to, or start fresh from! 

---
title: "Getting Started with R for Water Resources Data Science"
description: | 
  Unleashing your potential to work with any kind of data...
output: 
...
The goals of this training are to expose you to fundamentals and to develop an appreciation of what is possible with this software. Importantly, we hope to instill the idea that data science should be transferable and reproducible, no matter the specific tools (i.e., R) or the dataset that is used. It is a general framework and mindset. An introductory course will not make you an expert, but hopefully it will draw you in and make you excited about using these skills and approach in your own work, because it will make you more efficient and less stressed when faced with messy real-world data!
...
Computer literacy is now essential in all aspects of the working world. The ability to reproduce a set of common or repetitive tasks in an efficient and reliable manner is the root goal of reproducibility. Whether in science, management, or other fields, the skills required to increase productivity, maintain transparency, and feel competent and able to tackle multiple tasks all boil down to fairly basic tools of data science and project management.

Data management skills are needed for entering data without errors, storing it in a usable way, and extracting key aspects of the data for analysis. Basic programming is required for everything from accessing and managing data, data visualization, to statistical analysis and modeling. Data science is ultimately the skills and tools that allow for acquisition, processing, analyzing, communicating, and maintaining data. Of any type! 

# Course Objectives

This course's main objective is to provide students with a introduction into the power and possibility that a free and reproducible programming language such as [R](https://www.r-project.org/about.html) provides by demonstrating how to import, explore, visualize, and communicate different types of data. Using water resources based examples, this course will guide students through basic data science skills and strategies for continued learning and use of R. [R](https://www.r-project.org/) is a language for statistical computing as well as a general purpose programming language. Increasingly, it has become one of the primary languages used in data science and for data analysis across many of the natural sciences. This course will provide lessons in:
...
There are many programming languages available and each has it's specific benefits. R was originally created as a statistical programming language but now it is largely viewed as a 'data science' language. R is also an open-source programming language - not only is it free, but this means anybody can contribute to it's development. Furthermore, R has powerful and nearly limitless plotting/visualization functionality, and you can adjust nearly any aspect of a graph to communicate your data effectively.

 As Hadley Wickham states:
...
We recognize that the learning curve might be steeper than with other software you may use (i.e., Office software). However, with R the results of your analysis do not rely on remembering a series of point and clicks, but instead a series of written commands (`code`) that you can quickly re-run, revise, and re-use with many other datasets. We save these series of commands as a `script`, which means you can share your analysis with anyone else (or your future self!), and the steps you used are transparent and reproducible. 

<aside>
```{r rfirst, out.width='50%', fig.cap="R is hard at first...but gets easier! Artwork by @allison_horst"}
...
Let's get familiar with RStudio before we go on.  If you haven't done so already, download and install RStudio from the link above (version 1.4.1103).  After it's installed, find the RStudio shortcut and fire it up.  You should see something like this:
...
The first part of RStudio that we will be working in is called the *Console*. This is the part of the application that tells you what `code` R is running. We can use many of the same commands as you would be able to do in a calculator or in Microsoft Excel.
...
You might notice is no output in the **Console** for the lines of code you have just run. Instead, they have been stored in your R **Environment** in the top right pane of your RStudio window. Click on that tab and take a look! Because they have been stored, you can run these variables by typing their name in the **Console** and pressing `Enter`.
...
Immediately below the **Environment** is the third section of RStudio, where all of your **Packages** are stored. The base or core installation of R is quite powerful, but because R is open-source, there are thousands of packages (or essentially pieces of code we can use) that extend the capability of R dramatically, from statistical analysis, to modeling, to geospatial mapping, to website and document creation. Packages are the collection of code or *functions* that are a standardized way of extending R with new methods, techniques, and programming functionality.
...
In order to use a package, you must load it into your current workspace. This is sometimes called *attaching* a package, but we prefer to avoid clicking whenever possible, so we'll load this library using the `library()` function. Type the following into your **Console** and press `Enter`:
...
The remaining tabs in this pane allow you to see:
 ...
 - **Files** that are associated with your code, 

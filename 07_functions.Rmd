...
cntys <- USAboundaries::us_counties(states = "ca", resolution = "high")
sac_eldor_plac <- cntys %>% filter(name %in% c("Placer","Sacramento", "El Dorado"))

# this is cool but may need to be saved for advanced course..
# including code here to show a map of potential WQ sites we can use for joins lesson
# can split/show whatever pieces are useful later
...
# make data.frame of NWIS stations we want to use:
nwis_sites <- amer_ds_nimbus$DM_nwissite %>% st_drop_geometry()
...
Let's go ahead and save (export) our data so we can use it for future lessons.

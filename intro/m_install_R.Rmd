...
Go to [CRAN and download](https://cran.rstudio.com/bin/windows/base/) the R installer for Windows. Make sure to choose the latest stable version (v4.1.0 as of May 2021). Download the <font color="#0072B2">`.exe`</font> file and double click to install.
...
![](images/rstudio_console.png#inline-img "RStudio console")
 ...
 - **Verify your installed R version is `v4.1.0`**: Ideally you should be running the latest stable release (`v4.1.0` as of May 2021). If you have an older version, please install the newest version using the instructions above.
...
sf         1.0-1  1.0-2             FALSE
mapview   2.10.0 2.10.1              TRUE
...
To install {`sf`} successfully from source, Windows users will need to install [Rtools](https://cran.r-project.org/bin/windows/Rtools/). Download the 64-bit compiler.

After installing [RTools](https://cran.r-project.org/bin/windows/Rtools/), one final step remains...adding RTools to your path. This may require administrative privileges.

**_Why do we need to do this?_**

What does this mean? Why do we need to do it? RTools is like a translator for your computer, and it helps translate code and build software (R packages). Before your computer can use it, we need to tell R (on your computer) where the RTools you just installed is actually located. Once we add this path (the location of the directory where RTools lives), the computer can use it when it is needed.

**_To Add RTools to your Path_**

Good news is we can typically do this within R/RStudio without too much trouble. Try this:

 1. Open RStudio
 2. Navigate to the **Console** panel
 3. Right after the `>` prompt, paste the following line and hit `Enter` or `Return`
    
      `writeLines('PATH="${RTOOLS40_HOME}\\usr\\bin;${PATH}"', con = "~/.Renviron")` 

 4. Test that it worked. Close down RStudio, reopen it, and in the **Console** after the `>` prompt, paste the following: 

      `Sys.which("make")`

 5. We should see something like this printed out:

      `# "C:\\rtools40\\usr\\bin\\make.exe"`

Now you should be able to install or "*compile*" packages in R in Windows. Proceed with installation instructions for additional packages! 
...
We'll be working with a few different datasets for this workshop (see [here](https://github.com/richpauloo/r4wrds-data)). We've zipped all these files into a single folder, which can be downloaded [here](https://osf.io/d7qse/download). Bear in mind this **`.zip`** file is ~122 MB in size, and may take a few minutes to download depending on internet connections.

 - [**Download the zipped file of all intro course data**](https://osf.io/d7qse/download).

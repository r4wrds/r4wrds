...
However, a few simple tips about how to structure and use spreadsheets can make all the difference. There will still always be messy datasets to face, but learning how to more effectively use spreadsheets *will* make life easier and more reproducible, and hopefully make these encounters less daunting.

Let's go over what tidy data should look like, how to wrangle a messy real-life spreadsheet, and understand the difference between human readable and computer readable information.

# What is Tidy Data?

Ultimately spreadsheets (or any list or table of data) are attempting to take **human readable** data and turn it into ***computer readable*** data. The problem is we often like to add information that is only **human readable** (i.e., colors in spreadsheet). These bits of "flair" may be nice to visualize things, but they happen at the wrong stage. Computers will only interpret the literal data pieces from each cell, so if we want additional information to be converted, we need to store that information in a **computer readable** context.

Typically when we look at data, we want it to be in a "tidy" format. To be in a tidy format, we want to have each **column** as a unique variable, and each **row** as a unique observation of data. Furthermore, any information we want the computer to retain should be stored as `variables` and not in side comments or coloring. This makes it possible for a computer to easily interpret these data, and avoids any unnecessary conversions or loss of data.
...
include_graphics(here::here("images", "messy_spreadsheet_1.png"))
...
While there is a potential risk to accidentally adding a space in a blank cell, the simple and easiest approach for missing or `NA` data is to leave cells blank. This is generally consistently interpreted by computer programming languages and avoids the need to remember which code was used to signify `NA`. This also means that a **`0`** is a **`0`**. Any data that is **`0`** should be encoded as such, and not left blank.


# Pivoting

When we enter data in a spreadsheet, the data is typically oriented in a ***wide*** format. This makes entry easier and is generally easier for us to see and explore the data. While ***wide*** data is good for entry, ***long*** data is much better for analysis and visualization. In particular, in `R`, the `{ggplot}` package prefers data in a ***long*** format, which facilitates easier filtering, grouping, and faceting of the data.

## Wide to Long

Let's use the [**`ces3results.xlsx`**](https://github.com/ryanpeek/r4wrds/blob/main/data/calenviroscreen/ces3results.xlsx?raw=true) [CalEnviroscreen data](https://oehha.ca.gov/calenviroscreen), which we can read in and look at the column names and data types. By default, `read_xlsx()` reads in the first sheet. See the [import and export module](m_importing_and_exporting_data.html) for more details.
...
Note the different data classes here...even though all columns between `Ozone` and `Pop. Char. Pctl` should be numeric, there are a number of "`char`" data. This is because one of the issues with messy spreadsheets we discussed above. If we were to open the spreadsheet and scroll down through these columns, we may notice there are `NA`'s sprinkled throughout. When importing these data into R, those `NA` are interpreted as character class, and thus convert the entire column of numeric data to a character class as well. 
...
include_graphics(here::here("images", "dplyr_across.png"))
```
...
ces_long <- pivot_longer(data = ces,
                         ...
                         names_to = "CES_input_variables",
                         # name of column we create for pivoted values
                         values_to = "CES_input_values")
...
ces_wide <- pivot_wider(data = ces_long,
                        names_from = "CES_input_variables",
                        values_from = "CES_input_values"
                        )

```


There are many additional options within the {tidyr} `pivot` functions. We recommend further reading on [*pivoting at R4DS*](https://r4ds.had.co.nz/tidy-data.html#pivoting) and the [*`{tidyr}` pivoting vignette*](https://tidyr.tidyverse.org/articles/pivot.html), as well as checking out the `separate()` and `unite()` functions from the same package.
...
include_graphics(here::here("images", "tidydata_6.jpeg"))
```

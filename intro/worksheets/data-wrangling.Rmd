...
```{r download-sf, echo=FALSE, eval=FALSE}
...
However, we can download the data directly and use it.

```{r rm-shp-download, eval=TRUE, echo=FALSE}

load(here("worksheets/CES3June2018Update.rda"))

```


To import a shapefile into `R`, we need to use the `{sf}` package and `st_read`. Go ahead and run this code after you've downloaded the data.

```{r import-sf, echo=TRUE, eval=FALSE}
# import to R as sf
library(sf)
shpfile <- st_read(here("worksheets/CES3_shp/CES3June2018Update.shp"), quiet = TRUE) %>% 
  st_transform(4326)
...
Now we have a few datasets, we often want to filter or select data. Let's filter to the zip code with the **most** groundwater stations in a single zip code from `stations` dataset, and then plot it with our spatial dataset.
...
shpfile %>% 
  # we can use %in% for one or more items
  filter(ZIP %in% 95693) -> shp_trim 
...
shpfile %>% 
  # we can use == for a single item
  filter(ZIP == 95693) -> shp_trim 
...
```{r ggplot-sf, echo=TRUE, exercise = TRUE}

ggplot() + 
  geom_sf(data=___, aes(___=as.factor(tract))) + 
  scale_fill_viridis_d("Census Tract") +
  labs(x="Lon", y="Lat", title="Zip Code 95693 Groundwater Stations") + geom_sf(data=stations_sf, fill="blue"), pch=21)

```

```{r ggplot-sf-solution, echo=TRUE}

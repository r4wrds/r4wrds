...
## `group_by` and `summarize` into new data.frame
...
What happened? `group_by()` will group the data based on whatever variable or variables you include within that function. Once our `data.frame` is grouped, there are many options and functions we can apply, including things like `count()`, `tally()`, and `summarize()`.
...
We can add additional groups or columns depending on what we are interested in. Here we add another group (`WELL_USE`), but let's filter out all `WELL_USE` that are blank (""), *Unknown*, or *Other*, and arrange by `total_records` and `WELL_USE`. We'll also add `facets` to our ggplot!

```{r grpby4, eval=T, echo=T}

stations %>% 
  filter(!WELL_USE %in% c("Unknown", "Other", "")) %>%
  group_by(COUNTY_NAME, WELL_USE) %>% 
  summarize(total_records = n()) %>% 
  arrange(desc(total_records)) %>% 
  #head(20) %>% # return the first 10 rows 
  ggplot() + geom_col(aes(x=COUNTY_NAME, y=total_records)) +
  labs(title="Groundwater Stations in CA") +
       #subtitle="Top 10 Counties") +
  facet_wrap(.~WELL_USE, scales = "free")

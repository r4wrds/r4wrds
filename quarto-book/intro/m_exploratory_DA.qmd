...
```{r, eval=TRUE, echo=FALSE, out.width='80%', fig.cap="*Illustration by @allison_horst for Dr. Julia Lowndes useR!2019 keynote.*"}
...
```{r gw-model, echo = FALSE, out.width="100%", fig.cap="*Wells at different locations and depths in an aquifer, from [Wikimedia Commons](https://commons.wikimedia.org/wiki/File:Floridan_block_diagram.png).*"}
...
First we need to load our data, which we created in the module on [joins and binds](m_joins.qmd), and then inspect what's there.
...
An EDA may generate tables, visualizations, text, and code that all encapsulate the greater meaning that you've derived from the data. An excellent, R-centric way to share the tables, visualizations, text, and code that result from your EDA is to use an RMarkdown (`.Rmd`) document, which is the topic of the [next module](m_intro_Rmarkdown.qmd).
